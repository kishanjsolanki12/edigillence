...
@section('content')
  ...
  <div class="content-wrapper">
      <div class="container-full">
        ...
        <!-- Main content -->
        <form method="POST" action="{{ route('technologies.update', $technology) }}" class="form" novalidate data-parsley-validate="" enctype="multipart/form-data">
                                            ...
                                            <select class="form-control appearance-none focus:shadow-outline" name="category_id" id="category_id" >
                                                <option disabled value="0">Select Category</option>
                                                    @foreach($categories as $category)
                                                        <option {{ in_array($category->id,$selectedCategory) ? 'selected' : '' }} value="{{ $category->id }}">{{ $category->title }}</option>
                                                    @endforeach
                                            </select>
                                            @error('category_id')
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div class="input-block">
                                            ...
                                            @enderror
                                            @if($technology->image!='')
                                                <img src="{{ Storage::disk()->url('technologies/'.$technology->image) }}" class="img-fluid img-thumbnail" style="width:auto;height:150px;">
                                            @endif
                                        </div>
                                    </div>
                                </div>
                                E<div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div class="form-check">
                                            <input type="radio" name="status" id="is_publish_1" class="form-check-input focus:shadow-outline @error('status') is-invalid @enderror" value="1" required  {{ ($technology->status=="1")? "checked" : "" }}> 
                                            <label for="is_publish_1" class="form-check-label">Yes</label>
                                            <input type="radio" name="status" id="is_publish_0" class="form-check-input focus:shadow-outline @error('status') is-invalid @enderror" value="0" required {{ ($technology->status=="0")? "checked" : "" }}> 
